...
VARS = json.load(fp=open(file="utils\\VARs_HR.json", encoding="utf-8"))
...
class LLMResponsePostProcessor:
    ...
    def process_extract_vars(self, response: Any) -> Any:
    ...
    def process_missing_vars(self, response: Any) -> Any:
    ...
    def process_slave_queries(self, response: Any) -> Any:
    ...
    def process_api_evaluator(self, response: Any) -> Any:
    ...
    def process_chit_chat(self, response: Any) -> Any:
    ...
    def process_rag(self, response: Any) -> Any:
...
class LLMAgent(Prompts, LLMResponsePostProcessor):
    ...
    def llm_call(self, query: str, use: str) -> Any:
...
class ChatBotBackend(LLMAgent):
    ...
    def __init__(
        ...
        self.current_subquery: Tuple[str, str] = ("", "")
    ...
    async def resolve_slave_queries_agentic_api_dependencies(self):
        ...
        # print all the values of each api agent
        print("+++++++++++++++API Agents:+++++++++++++++")
        for agent in self.api_agents:
            print(agent.dict())
            print("------------------")
        print("+++++++++++++ API DATA ++++++++++++++++++")
    ...
    def update_memory(self, subquery: str, keyword: str, user_response: str):
        llm_query = f"Query: {subquery} \n keyword: {keyword} \n Value: {user_response}\nList of Variables: {', '.join(self.missing_vars)}"
        response: Dict[str, str] = self.llm_call(llm_query, use="extract_vars")
        print("+++++++++++++++ Extracted Vars +++++++++++++++")
        print(response)
    ...
    def execute_apis(self):
        for api_name in self.apis:
            ...
            api_response: Dict[str, Any] = tools[api_name](
                **self.memory.api_data, **self.memory.user_data
            )
            self.memory.api_data.update(api_response)

    def call_rag_agent(
        self, query: str, var_data: str, recreate_vector_db: bool = False
    ...
    def api_evaluator(self) -> Tuple[str, str, Any]:
        """
    ...
    async def get_api_response(
        ...
        if subquery_response:
            ...
            self.update_memory(
                subquery=self.current_subquery[1],
                keyword=self.current_subquery[0],
                user_response=subquery_response,
            )

        if len(self.subqueries) > 0:
            self.current_subquery = self.subqueries.pop(0)
            return (self.current_subquery[0], self.current_subquery[1], None), 1
